...
// PRIO_USER: Used for all processes owned by a specific user.
    //priority  ranges from -20 to 19 and -20 is higest priority and 19 is lowest.in unix lowest the value the highest the priority
    if (old_priority == -1) {
    ...
    int decrement = -25; // Priority adjustment (positive value lowers the priority)
    if (nice(decrement) == -1) {
