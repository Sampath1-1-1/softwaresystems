...
int main() {
    ...
    if (waitpid(pid2, &status, 0) == -1) {
        //the function is used to wait for a specific child process
        perror("Failed to wait for Child 2");
        exit(EXIT_FAILURE);
    }

    if (WIFEXITED(status)) {
        //    This macro returns a non-zero value (true) if the child process terminated normally, i.e., it called exit() or returned from main(). 
        // If the child process terminated in any other way (e.g., due to a signal), it will return zero (false).
