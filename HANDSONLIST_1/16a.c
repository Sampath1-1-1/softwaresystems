...
int main(){ 
    ...
    lck.l_type=F_RDLCK;
    lck.l_whence=SEEK_SET;//l_whence defines the reference point in the file (e.g., beginning, current position, or end).
// l_start defines how far from that reference point the lock should start.
    lck.l_start = 0;
...
// l_len = 50: The lock extends 50 bytes from the starting point (byte 10).
    ...
    lck.l_pid=getpid();
    lck.l_pid = getpid();// ensures that the l_pid field of the lock structure contains the PID of the process
    // that is trying to acquire the lock. This PID can be used later to identify which process owns the lock.
    ...
    getchar();
    //getchar() is frequently used to pause the execution of a program until the user presses the Enter key.
...
//         F_SETLK attempts to acquire the specified lock (either a read lock F_RDLCK or a write lock F_WRLCK) immediately.
// F_SETLKW

//     Blocking Locking:
//         F_SETLKW also attempts to acquire the specified lock (either F_RDLCK or F_WRLCK), but with one key difference:
//          it blocks (waits) if the lock cannot be immediately obtained because another process holds an incompatible lock.